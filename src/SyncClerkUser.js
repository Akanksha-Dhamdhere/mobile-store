  ...
  useEffect(() => {
    if (isSignedIn && user?.primaryEmailAddress?.emailAddress) {
      ...
      fetch("/api/users", {
        ...
        .then(res => res.json())
        .then(userData => {
          devLog("Clerk user fetched from backend:", userData);
        ...
        .then(res => res.json())
        .then(loginData => {
          devLog("Clerk user session established:", loginData);
        })
        .catch(err => {
          devError("Error syncing, fetching, or logging in Clerk user:", err);
        });
    }
  }, [isSignedIn, user]);
