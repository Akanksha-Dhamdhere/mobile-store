...
export default function OrderNow() {
  ...
  // show order error message in UI when set
  const OrderErrorBanner = () => orderError ? (
    <div className="my-4 p-3 bg-red-100 text-red-700 rounded">{orderError}</div>
  ) : null;
  ...
  const handlePlaceOrder = async () => {
    if (payment.method === 'UPI') {
      ...
      try {
        ...
        const options = {
          key: process.env.REACT_APP_RAZORPAY_KEY_ID || 'rzp_test_uNl7K4UX0VyScu',
