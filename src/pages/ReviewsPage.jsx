...
const ReviewPage = () => {
  ...
  const handleDelete = async (review) => {
    // Only allow deletion of user reviews or embedded reviews when ids exist
    const ok = window.confirm('Are you sure you want to delete this review? This action cannot be undone.');
    if (!ok) return;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {sortedReviews.length > 0 ? (
          sortedReviews.map((review, idx) => (
            <ReviewCard key={review.id || review._id || idx} review={review} onDelete={handleDelete} />
          ))
        ) : (
