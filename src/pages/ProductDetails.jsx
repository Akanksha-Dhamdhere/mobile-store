...
function ProductDetails() {
  ...
  const handleAddToCart = async () => {
    ...
    devLog('[AddToCart] Sending:', { prodId, model, user: user?.email });
  ...
  const handleRemoveFromCart = async () => {
    ...
    devLog('[RemoveFromCart] Cart contents:', cart);
    devLog('[RemoveFromCart] Attempting to remove:', { prodId, model, user: user?.email });
    // Remove from cart robustly
    try {
      const api = await import('../utils/api');
      await api.removeFromCart(prodId, user?.token, model);
    } catch (err) {
      devError('[RemoveFromCart] Error:', err);
    }
    if (!user || !user._id) { setCart([]); setIsInCart(false); return; }
    const cartData = await fetchCart(user._id);
    setCart((cartData && cartData.data) || []);
    ...
    const found = ((cartData && cartData.data) || []).some((c) => {
      ...
      if (match) devLog('[RemoveFromCart] Still found in cart:', { cid, cmodel });
      return match;
    });
    setIsInCart(found);
    window.dispatchEvent(new Event('cartWishlistUpdated'));
  ...
  const handleAddToWishlist = async () => {
    ...
    const model = getModel(product);
    const prodId = product._id || product.id;
    devLog('[AddToWishlist] Sending:', { prodId, model, user: user?.email });
  ...
  const handleRemoveFromWishlist = async () => {
    ...
    const prodId = product._id || product.id;
    try {
      devLog('[RemoveFromWishlist] Attempting to remove:', { prodId, model, user: user?.email });
      await updateWishlist(prodId, "remove", user?.token, model);
    } catch (err) {
      devError('[RemoveFromWishlist] Error:', err);
    }
    if (!user || !user._id) { setWishlist([]); setIsInWishlist(false); return; }
    const wishlistData = await fetchWishlist(user._id);
    setWishlist((wishlistData && wishlistData.data) || []);
    ...
    const found = ((wishlistData && wishlistData.data) || []).some((w) => {
      ...
      if (match) devLog('[RemoveFromWishlist] Still found in wishlist:', { wid, wmodel });
      return match;
    });
      ...
      <div className="mt-10">
        ...
        {user && user.email ? (
          <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg shadow-lg">
            ...
            {showToast && <p className="text-green-600 mt-2">Review submitted</p>}
          </div>
        ) : (
