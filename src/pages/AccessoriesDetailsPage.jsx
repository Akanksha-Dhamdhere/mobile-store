...
function AccessoriesDetailsPage() {
  ...
  const handleAddReview = async () => {
    ...
    if (!userReview || userReview.trim().length === 0) {
    ...
    try {
      ...
      const backendUrl = process.env.REACT_APP_BACKEND_URL || 'http://localhost:5000';
      const response = await fetch(`${backendUrl}/api/accessories/${id}/reviews`, {
        method: "POST",
        headers: { 
      ...
      const refetchUrl = `${backendUrl}/api/accessories/${id}`;
      const refetchRes = await fetch(refetchUrl);
      const refetchResponse = await refetchRes.json();
      
      // Handle both response formats
      const refetchData = refetchResponse.data || refetchResponse;
      
      if (refetchData) {
        setAccessory(refetchData);
        setAvgRating(refetchData.avgRating || null);
        if (refetchData.reviews && Array.isArray(refetchData.reviews)) {
          setReviews([...refetchData.reviews]);
        } else {
          setReviews([]);
        }
      }
